    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            {isTeacher 
              ? "Monitor the latest steps of all projects uploaded by students." 
              : "Manage your active projects and share your progress with your mentor."}
          </p>
        </div>
      ...
      {projects.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 bg-slate-900/30 border border-slate-800/50 rounded-2xl border-dashed">
          ...
          <h3 className="text-lg font-medium text-white mb-2">No projects found yet</h3>
          <p className="text-slate-400 text-center max-w-sm mb-6">
            {isTeacher 
              ? "Students who joined the system haven't started any projects yet." 
              : "Add your first project to start project management."}
          </p>
          {!isTeacher && (
            <Link href="/dashboard/projects/new" className="text-indigo-400 font-medium hover:text-indigo-300 transition-colors">
              Add a project now &rarr;
            </Link>
          )}
        </div>
      ) : (
