...
export default function ChatWindow({ currentUser, selectedUser }: ChatWindowProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-900/40 backdrop-blur-2xl rounded-r-3xl overflow-hidden shadow-soft relative">
      {/* Header */}
      <div className="p-5 border-b border-slate-800/60 bg-slate-900/40 backdrop-blur-xl shrink-0 z-10 shadow-sm">
          ...
          messages.map((msg, index) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} ${isLastFromSame ? 'mt-1' : 'mt-6'}`}>
                <div 
                  className={`max-w-[75%] md:max-w-[65%] px-5 py-3 shadow-md ${
                    isMine 
      ...
      {/* Input */}
      <div className="p-4 sm:p-5 border-t border-slate-800/60 bg-slate-900/60 backdrop-blur-xl relative z-20">
          ...
          <div className="absolute bottom-[calc(100%+8px)] left-5 mb-2 w-72 bg-slate-800/95 backdrop-blur-3xl border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden z-30">
            ...
            <ul className="max-h-48 overflow-y-auto custom-scrollbar p-1">
              {projects.map(p => (
                <li 
                  key={p.id} 
                  className="px-4 py-3 hover:bg-indigo-500/10 rounded-xl cursor-pointer text-sm font-medium text-slate-200 transition-all hover:pl-5 border border-transparent hover:border-indigo-500/20 m-1"
                  onClick={() => selectMention(p)}
                >
                  <FolderRoot className="inline-block w-4 h-4 mr-2 text-indigo-400" />
                  {p.title}
                </li>
              ))}
            </ul>
        ...
        <form onSubmit={handleSend} className="flex gap-3">
          <input
            ...
            placeholder="Type your message... (Use @ to mention a project)"
