...
function FilterDropdown({
  ...
  const statusMeta: Record<string, { icon: React.ReactNode; color: string }> = {
    "To Do":     { icon: <Minus className="w-3 h-3" />,       color: "text-slate-500 dark:text-slate-400"  },
  ...
  return (
    <div className="w-64 bg-white dark:bg-slate-900 border border-slate-200 rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50 dark:bg-slate-950/60">
        <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Filters</span>
        {total > 0 && (
          <button
            onClick={onClear}
          ...
          {projects.map((project, i) => (
            ...
            >
              <div className="min-w-0">
                <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate leading-tight">{project.title}</p>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl border transition-all shrink-0 ${
                activeCount > 0
                  ...
                  : "text-slate-600 dark:text-slate-300 border-slate-200 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-950"
              }`}
            >
        ...
        {activeCount > 0 && (
          ...
          >
            ...
            <button
              ...
              className="text-xs font-semibold text-slate-400 hover:text-slate-600 dark:text-slate-300 transition-colors"
            >
              Clear all
            </button>
          </motion.div>
