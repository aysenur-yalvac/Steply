...
const WatchlistItem = memo(function WatchlistItem({
  ...
  return (
    ...
    >
      <h3 className="font-bold text-slate-800 dark:text-slate-200 dark:text-slate-100 group-hover:text-dusty-rose transition-colors pr-6">
        {project.title}
      </h3>
      <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Student: {project.studentName}</p>
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-slate-200/50 dark:border-slate-800/50 bg-slate-50/30 dark:bg-slate-900/30">
              <h2 className="text-xl font-bold flex items-center gap-2 text-slate-800 dark:text-slate-200 dark:text-white">
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
              {watchlist.length === 0 ? (
                <p className="text-slate-500 dark:text-slate-400 text-center mt-10">You have no watched projects yet.</p>
              ) : (
