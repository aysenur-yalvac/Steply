...
| BE-068 | Backend_Agent | Kök neden: `profiles.institution`'a e-posta yazılabiliyordu (serbest metin girişi/eski veri) → Okulum sayfası banner'ında okul adı yerine email görünüyordu. `sanitizeInstitution()` (`lib/utils.ts`) eklendi; `updateProfileAction` artık '@' içeren değeri hiç kaydetmiyor | Tamamlandı | QA: OK |
| FE-061 | Frontend_Agent | `school/page.tsx`: institution sanitize edilerek okunuyor, geçersizse banner "Henüz bir okul seçilmedi" gösteriyor (öğretmen/öğrenci listeleri otomatik boşalıyor). Aynı guard `user/[id]/page.tsx` (public profil), `dashboard/profile/page.tsx`, `UserSearch.tsx`, `SettingsClient.tsx` (client-side toast) için de eklendi | Tamamlandı | QA: OK |
| FE-062 | Frontend_Agent | `PhotoLightbox.tsx`: modal `max-w-sm`(384px)→`max-w-[min(90vw,36rem)]`(~576px, mobilde taşmıyor); görsel `max-h-[75vh]`→`max-h-[80vh]`; isim yazısı ve kapatma butonu yeni boyuta göre büyütüldü. Okulum sayfası + öğrenci/öğretmen kartlarının tamamını kapsıyor (tek paylaşılan bileşen) | Tamamlandı | QA: OK |
| FE-063 | Frontend_Agent | `PhotoLightbox.tsx`: zoom & pan eklendi — wheel/touchpad pinch zoom (native non-passive preventDefault ile arka plan kayması engellendi), 2 parmak pinch + 1 parmak pan (touch), masaüstünde tıkla-sürükle pan, Zoom In/Out/Sıfırla butonları + yüzde göstergesi. Fotoğraf değişince veya modal kapanınca scale/position otomatik sıfırlanıyor | Tamamlandı | QA: OK |
